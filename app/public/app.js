(() => {
  ...
  function applyAvatarVisual(el, c) {
    if (c.photo) {
      // Set the whole shorthand in one go — assigning `backgroundImage`
      // and then `background` separately makes the later shorthand
      // assignment reset the image back to nothing, which was the bug.
      el.style.background = `url('${c.photo}') center / cover no-repeat`;
      el.textContent = '';
    } else {
